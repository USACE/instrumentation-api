...
if [ "$1" = "watch" ]; then
    ...
    if [ "$2" = "mock" ]; then
        ...
        DOCKER_BUILDKIT=1 AUTH_JWT_MOCKED=false $COMPOSECMD -f docker-compose.dev.yml -f docker-compose.auth.yml watch
    else
    ...
    if [ "$2" = "mock" ]; then
        ...
        DOCKER_BUILDKIT=1 AUTH_JWT_MOCKED=false $COMPOSECMD -f docker-compose.auth.yml up -d --build
    else
