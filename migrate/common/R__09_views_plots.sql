...
--     LEFT JOIN LATERAL (
--         SELECT json_agg(
--             to_jsonb(tr) || jsonb_build_object('name', i.name || ' - ' || ts.name || ' (' || u.name || ')'))::json
--         ) as items
...
--     ) traces ON true
--     LEFT JOIN LATERAL (
--         SELECT json_agg(to_json(*)) AS items
--         FROM plot_configuration_custom_shape
--         WHERE pc.id = cs.plot_configuration_id
--     ) cs ON true
-- );
--
