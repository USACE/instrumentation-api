...
type InclinometerMeasurementService interface {
...
type inclinometerMeasurementService struct {
	db *model.Database
	*model.Queries
}

func NewInclinometerMeasurementService(db *model.Database, q *model.Queries) *inclinometerMeasurementService {
	return &inclinometerMeasurementService{db, q}
}
...
func (s inclinometerMeasurementService) CreateOrUpdateInclinometerMeasurements(ctx context.Context, im []model.InclinometerMeasurementCollection, p model.Profile, createDate time.Time) ([]model.InclinometerMeasurementCollection, error) {
...
func (s inclinometerMeasurementService) CreateTimeseriesConstant(ctx context.Context, timeseriesID uuid.UUID, parameterName string, unitName string, value float64) error {
	tx, err := s.db.BeginTxx(ctx, nil)
	if err != nil {
		return err
	}
