...
var plotConfigObjectSchema = gojsonschema.NewStringLoader(plotConfigSchema)

var plotConfigArraySchema = gojsonschema.NewStringLoader(fmt.Sprintf(`{
    "type": "array",
    "items": %s
}`, plotConfigSchema))
	...
	tests := []HTTPTest{
		{
			...
			ExpectedSchema: &plotConfigObjectSchema,
		},
		{
			...
			Method:         http.MethodGet,
			ExpectedStatus: http.StatusOK,
			ExpectedSchema: &plotConfigArraySchema,
		},
		{
