...
func (h ApiHandler) allInclinometerTimeseriesBelongToProject(db *sqlx.DB, mcc *model.InclinometerMeasurementCollectionCollection, projectID *uuid.UUID) (bool, error) {
	// timeseries IDs
...
func (h ApiHandler) ListInclinometerMeasurements(c echo.Context) error {
	...
	im, err := model.ListInclinometerMeasurements(db, &tsID, &tw)

	if err != nil {
		return echo.NewHTTPError(http.StatusInternalServerError, err.Error())
	}

	cm, err := model.ConstantMeasurement(db, &tsID, "inclinometer-constant")
	if err != nil {
		return echo.NewHTTPError(http.StatusBadRequest, err.Error())
	}

	for idx := range im.Inclinometers {
		values, err := model.ListInclinometerMeasurementValues(db, &tsID, im.Inclinometers[idx].Time, cm.Value)
		if err != nil {
			return echo.NewHTTPError(http.StatusInternalServerError, err.Error())
		}
...
func (h ApiHandler) CreateOrUpdateProjectInclinometerMeasurements(c echo.Context) error {
	...
	stored, err := model.CreateOrUpdateInclinometerMeasurements(db, mcc.Items, p, time.Now())
	if err != nil {
		return echo.NewHTTPError(http.StatusBadRequest, err.Error())
	}
	...
	if len(mcc.Items) > 0 {
		cm, err := model.ConstantMeasurement(db, &mcc.Items[0].TimeseriesID, "inclinometer-constant")
		if err != nil {
			return echo.NewHTTPError(http.StatusBadRequest, err.Error())
		}

		if cm.TimeseriesID == uuid.Nil {
			err := model.CreateTimeseriesConstant(db, &mcc.Items[0].TimeseriesID, "inclinometer-constant", "Meters", 20000)
			if err != nil {
				return echo.NewHTTPError(http.StatusBadRequest, err.Error())
			}
		}
...
func (h ApiHandler) DeleteInclinometerMeasurements(c echo.Context) error {
	...
	if err := model.DeleteInclinometerMeasurements(db, &id, t); err != nil {
		return echo.NewHTTPError(http.StatusBadRequest, err.Error())
	}
