...
type InclinometerMeasurementCollectionCollection struct {
	Items []model.InclinometerMeasurementCollection
}
...
func (cc *InclinometerMeasurementCollectionCollection) UnmarshalJSON(b []byte) error {
	...
	case "OBJECT":
		var mc model.InclinometerMeasurementCollection
		if err := json.Unmarshal(b, &mc); err != nil {
			return err
		}
		cc.Items = []model.InclinometerMeasurementCollection{mc}
	default:
		cc.Items = make([]model.InclinometerMeasurementCollection, 0)
	}
	return nil
}

// ListInclinometersMeasurements returns a timeseries with slice of inclinometer measurements populated
func ListInclinometerMeasurements(db *sqlx.DB, timeseriesID *uuid.UUID, tw *model.TimeWindow) (*model.InclinometerMeasurementCollection, error) {

	mc := model.InclinometerMeasurementCollection{TimeseriesID: *timeseriesID}
...
func ListInclinometerMeasurementValues(db *sqlx.DB, timeseriesID *uuid.UUID, time time.Time, inclinometerConstant float64) ([]*model.InclinometerMeasurementValues, error) {
	constnat := fmt.Sprintf("%.0f", inclinometerConstant)
	v := []*model.InclinometerMeasurementValues{}
...
func CreateOrUpdateInclinometerMeasurements(db *sqlx.DB, im []model.InclinometerMeasurementCollection, p *Profile, createDate time.Time) ([]model.InclinometerMeasurementCollection, error) {
