...
func queryTimeseriesMeasurements(db *sqlx.DB, f *MeasurementsFilter) (MeasurementsResponseCollection, error) {
	...
	for idx, t := range tt {
		tt2[idx] = Timeseries{
			...
			TimeWindow:     model.TimeWindow{Start: f.After, End: f.Before},
		}
...
func ComputedInclinometerTimeseries(db *sqlx.DB, instrumentIDs []uuid.UUID, tw *model.TimeWindow, interval *time.Duration) ([]InclinometerTimeseries, error) {
	tt := make([]DBTimeseries, 0)
	sql := `
...
func (mrc *MeasurementsResponseCollection) GroupByInstrument(threshold int) (map[uuid.UUID][]model.MeasurementCollectionLean, error) {
	if len(*mrc) == 0 {
		return make(map[uuid.UUID][]model.MeasurementCollectionLean), fmt.Errorf(messages.NotFound)
	}

	tmp := make(map[uuid.UUID]map[uuid.UUID][]model.MeasurementLean)

	for _, t := range *mrc {
		if _, hasInstrument := tmp[t.InstrumentID]; !hasInstrument {
			tmp[t.InstrumentID] = make(map[uuid.UUID][]model.MeasurementLean, 0)
		}
		if _, hasTimeseries := tmp[t.InstrumentID][t.TimeseriesID]; !hasTimeseries {
			tmp[t.InstrumentID][t.TimeseriesID] = make([]model.MeasurementLean, 0)
		}
		for _, m := range t.Measurements {
			tmp[t.InstrumentID][t.TimeseriesID] = append(tmp[t.InstrumentID][t.TimeseriesID], model.MeasurementLean{m.Time: m.Value})
		}
	}

	res := make(map[uuid.UUID][]model.MeasurementCollectionLean)

	for instrumentID := range tmp {
		res[instrumentID] = make([]model.MeasurementCollectionLean, 0)

		for tsID := range tmp[instrumentID] {
			res[instrumentID] = append(res[instrumentID],
				model.MeasurementCollectionLean{
					TimeseriesID: tsID,
					Items:        model.LTTB(tmp[instrumentID][tsID], threshold),
				},
			)
		}
	}
...
func (mrc *MeasurementsResponseCollection) CollectSingleTimeseries(threshold int, tsID *uuid.UUID) (model.MeasurementCollection, error) {
	if len(*mrc) == 0 {
		return model.MeasurementCollection{}, fmt.Errorf(messages.NotFound)
	}

	for _, t := range *mrc {
		if t.TimeseriesID == *tsID {
			mmts := make([]model.Measurement, len(t.Measurements))
			for i, m := range t.Measurements {
				mmts[i] = model.Measurement{
			...
			return model.MeasurementCollection{TimeseriesID: t.TimeseriesID, Items: model.LTTB(mmts, threshold)}, nil
		}
	}

	return model.MeasurementCollection{}, fmt.Errorf("requested timeseries does not match any in the result")
}
