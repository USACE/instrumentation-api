...
func ListInstrumentConstants(db *sqlx.DB, id *uuid.UUID) ([]model.Timeseries, error) {
...
func CreateInstrumentConstants(db *sqlx.DB, tt []model.Timeseries) ([]model.Timeseries, error) {
	txn, err := db.Beginx()
	if err != nil {
		return nil, err
	}
